"""Verba × Digital Suvidha — AI voice calling agents (a Sahayak AI product).

FastAPI app:
  GET  /              -> the agent page (scenario picker: consult line / invite outreach)
...
app = FastAPI(title="Verba AI — voice agents for Digital Suvidha", lifespan=lifespan)
...
async def api_opening(password: str = Form(default=""), scenario: str = Form(default="consult"),
                      lang: str = Form(default="")):
...
def _crm_row(scenario: str, tool: str, args: dict) -> dict:
    ...
    if tool == "log_enquiry":
        ...
        bits = [a.get("service_interest"), a.get("business_type"),
                ("in " + a["city"]) if a.get("city") else None,
                a.get("notes")]
    ...
    if tool == "log_consultation_rsvp":
        ...
        bits = [a.get("preference_note"), a.get("notes")]
...
def _handlers_for(scenario: str, captured: dict, on_row=None) -> dict:
    """Tool handlers. log_enquiry/log_consultation_rsvp write a CRM row (and push it, for the
    WS path)."""
    ...
    return {
        ...
        "log_consultation_rsvp": lambda args: _save("log_consultation_rsvp", args),
    }
...
async def api_turn(
    ...
    scenario: str = Form(default="consult"),
    lang: str = Form(default=""),
...
async def _process_text(ws: WebSocket, state: dict, text: str, silent: bool = False):
    ...
    scenario = state.get("scenario", "consult")
    sc = scenario_of(scenario)
...
async def ws_endpoint(ws: WebSocket):
    await ws.accept()
    state = {"session_id": uuid.uuid4().hex, "contents": [], "scenario": "consult", "lang": ""}
